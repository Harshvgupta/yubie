...
class Fetch():
    def __init__(self):
        ...
        self.img_sub = rospy.Subscriber(
            "/spot_image", sensor_msgs.msg.Image, self.image_callback, queue_size=1)
        # Subscribe depth
        self.depth_sub = rospy.Subscriber(
            "/depth_array", std_msgs.msg.Float64MultiArray, self.depth_callback, queue_size=1)
        # Subscribe CameraInfo
        self.cam_info_sub = rospy.Subscriber(
            "/spot_cam_info", sensor_msgs.msg.CameraInfo, self.camera_callback, queue_size=1)

        self.ball_pos = rospy.Publisher(
            "ball_pose", geometry_msgs.msg.Pose, queue_size=1)
        self.ball_pos_viz = rospy.Publisher(
            "ball_pose_viz", visualization_msgs.msg.Marker, queue_size=1)
        self.final_pose_pub = rospy.Publisher(
            "final_ball", PoseStamped, queue_size=1)
    ...
    def extract_info(self, detections):
        ...
        print("bounding",self.boundingBoxes)
        return self.boundingBoxes, self.centroids
    ...
    def image_callback(self, msg):
        try:
            ...
            if len(centroids) > 0:
                ...
                print("c",centroids[0][0],centroids[0][1])
                self.x, self.y=self.centroids[0][0],self.centroids[0][1]
                self.get_obj_pose(self.x, self.y, self.depthc)
                # print(self.x,self.y,self.depthc)
    ...
    def depth_callback(self, msg):
        ...
        self.depthc = self.depth_array[int(
            self.centroids[0][0]), int(self.centroids[0][1])]
        # self.depthc = self.depth_array[int(
        #     190), int(346)]
    ...
    def get_world_pos(self, centroids, depthc):
        ...
        self.x = self.depthc * (self.centroids[0][0]-319.110595703125 / 332)
        self.y = self.depthc * (self.centroids[0][1]-236.84848022460938 / 331)
        x1 = 331 * (self.centroids[0][0]-319.110595703125 / self.depthc)
        y1 = 331 * (self.centroids[0][1]-236.84848022460938 / self.depthc)
        print("center", self.x, self.y,x1,y1)
